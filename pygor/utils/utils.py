...
def get_seq_rev_comp(seq):
    ...
    for nt in seq:
        if nt == "A":
            ...
            print("Nucleotide " + nt +
                  " not supported in get seq rev comp function")
    return rev_comp_seq


def get_str_asarray(full_str, dtype=float, boundaries_char=["(", ")"],
                    sep=','):
    """Returns subfields of string seperated by *sep* , remove boundary chars
    and convert to the provided type.
    ...
    if len(full_str) > (len(boundaries_char[0]) + len(boundaries_char[1])):
        ...
        elif full_str.find(boundaries_char[1]) != (
                len(full_str) - len(boundaries_char[1])):
        ...
        while next_comma_index != -1:
            full_array.append(
                dtype(full_str[comma_index + 1:next_comma_index]))
